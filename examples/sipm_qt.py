...
class SimulationChart(QWidget):
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        error_top = system_rect.bottom() + gap
        error_rect = QRectF(
            bounds.left(),
            error_top,
            bounds.width(),
    ...
    def _draw_panel(
        ...
        value_label_rect = rect.adjusted(8, 6, -8, -24 if show_time_labels else -6)
    ...
    def _draw_line(
        ...
        for sample_time, value in zip(times, values):
            x = plot_rect.left() + plot_rect.width() * (sample_time - time_min) / time_scale
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("pySimplePID SiPM Peltier control")
        ...
        self.model = SiPMModel()
        self.history = HistoryBuffer()
        self.time = 0.0
        self.model_dt = 0.1
        self.pid_elapsed = 1.0
        self.current_pid_output = 0.0
        self.current_peltier_drive = 0.0
        ...
        self.peltier_power_card = ValueCard("Peltier power")
        self.error_card = ValueCard("Temp error")

        self.target_input = self._spin_box(-30.0, 40.0, self.pid.target, 0.1)
        self.ambient_input = self._spin_box(
            -20.0,
            60.0,
            self.model.ambient_temperature,
            0.1,
        )
        self.sipm_heat_input = self._spin_box(
            0.0,
            2.0,
            self.model.sipm_heat_power,
            0.01,
        )
        self.max_peltier_input = self._spin_box(
            0.0,
            50.0,
            self.model.max_peltier_cooling_power,
            0.1,
        )
        self.conductance_input = self._spin_box(
            0.0,
            5.0,
            self.model.thermal_conductance_to_ambient,
            0.01,
        )
        self.area_input = self._spin_box(
            1.0,
            100.0,
            self.model.aluminium_area * 10000.0,
            1.0,
        )
        self.thickness_input = self._spin_box(
            0.1,
            10.0,
            self.model.aluminium_thickness * 1000.0,
            0.1,
        )
        self.kp_input = self._spin_box(0.0, 100.0, self.pid.kp, 0.1)
        self.ki_input = self._spin_box(0.0, 10.0, self.pid.ki, 0.01)
        self.kd_input = self._spin_box(0.0, 100.0, self.pid.kd, 0.1)
        self.pid_interval_input = self._spin_box(0.1, 30.0, 1.0, 0.1)

        controls = QFormLayout()
        controls.addRow("Target (C)", self.target_input)
        controls.addRow("Ambient (C)", self.ambient_input)
        controls.addRow("SiPM heat (W)", self.sipm_heat_input)
        controls.addRow("Max Peltier cooling (W)", self.max_peltier_input)
        controls.addRow("Ambient conductance (W/K)", self.conductance_input)
        controls.addRow("Aluminium area (cm2)", self.area_input)
        controls.addRow("Aluminium thickness (mm)", self.thickness_input)
        ...
        side_panel.addWidget(self.peltier_power_card)
    ...
    def step(self) -> None:
        ...
        if self.pid_elapsed >= self.pid_interval_input.value():
            self.current_pid_output = self.pid.update(
                self.model.current_temperature
            )
            self.current_peltier_drive = max(0.0, -self.current_pid_output)
            self.pid_elapsed = 0.0

        self.current_cooling_power = (
            self.current_peltier_drive
            * self.model.max_peltier_cooling_power
            / 100.0
        )
        temperature = self.model.step(
            peltier_cooling_power=self.current_cooling_power,
            dt=self.model_dt,
        )
        self.history.add(
            Sample(
                ...
                ambient_temperature=self.model.ambient_temperature,
        ...
        self.pid_output_card.set_value(f"{self.current_pid_output:.1f} %")
        self.peltier_power_card.set_value(f"{self.current_cooling_power:.2f} W")
    ...
    def reset(self) -> None:
        ...
        self.current_peltier_drive = 0.0
        self.current_cooling_power = 0.0
        self.temperature_card.set_value(f"{self.model.current_temperature:.2f} C")
        self.pid_output_card.set_value("0.0 %")
        self.peltier_power_card.set_value("0.00 W")
        reset_error = self.pid.target - self.model.current_temperature
    ...
    def _update_parameters(self) -> None:
        ...
        self.model.ambient_temperature = self.ambient_input.value()
        self.model.sipm_heat_power = self.sipm_heat_input.value()
        self.model.max_peltier_cooling_power = self.max_peltier_input.value()
        self.model.thermal_conductance_to_ambient = self.conductance_input.value()
        self.model.aluminium_area = self.area_input.value() / 10000.0
        self.model.aluminium_thickness = self.thickness_input.value() / 1000.0
